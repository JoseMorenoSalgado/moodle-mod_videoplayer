...
$headercallback = function($curl, string $header) use (&$headerbuffer, &$contentlength, &$remotecontenttype): int {
    ...
    } else if (preg_match('/^Last-Modified:\s*(.+)$/i', $trimmed, $matches)) {
        $headerbuffer['Last-Modified'] = trim($matches[1]);
    } else if (preg_match('/^ETag:\s*(.+)$/i', $trimmed, $matches)) {
        $headerbuffer['ETag'] = trim($matches[1]);
    }
    return $length;
};
...
if (!headers_sent()) {
    ...
    if (!empty($headerbuffer['Last-Modified'])) {
        header('Last-Modified: ' . $headerbuffer['Last-Modified']);
    }
    if (!empty($headerbuffer['ETag'])) {
        header('ETag: ' . $headerbuffer['ETag']);
    }
}
